...
int transform_gpu(unsigned char* image, histogram hist, int width, int height, int cpp) {

    // Load kernel source file.
    FILE* file;
    file = fopen("transform_kernel.cl", "r");
    if (!file) {
        printf("Could not load the kernel.\n");
        return 1;
    }
    ...
    // Allocate memory on device.
    cl_mem image_device = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, image_size * cpp * sizeof(unsigned char), image, &cl_status);
    cl_mem r_device = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, BINS * sizeof(unsigned int), hist.R, &cl_status);
    cl_mem g_device = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, BINS * sizeof(unsigned int), hist.G, &cl_status);
    cl_mem b_device = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, BINS * sizeof(unsigned int), hist.B, &cl_status);

    // Create kernel and add arguments.
    cl_kernel kernel = clCreateKernel(program, "transform", &cl_status);
...
int hist_equal_gpu(unsigned char* image, int width, int height, int cpp) {
    ...
    // Assign the new transformed value to each color channel of a pixel.
    transform_gpu(image, hist, width, height, cpp);
